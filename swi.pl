...
move_c :-
    c(C1,' '),
    o(ID,C1,C2,C3,C4,C5),
    c(C2,o), c(C3,o), c(C4,o), c(C5,' '),
    retract(c(C5,' ')),
    assert(c(C5,x)),
    write([C5,ID,'block right _ooo_']), nl,
    win(x).
...
move_c :-
    ...
    retract(c(C1,' ')),
    assert(c(C1,x)),
    write([C1,ID,'block left _ooo_']), nl,
    win(x).
...
move_c:-c(C1,' '),
    ...
    retract(c(C1,' ')),
    assert(c(C1,x)),
    write([C1,'double three fork']),now check the order or correct and what else is missing
